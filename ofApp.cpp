...
void ofApp::update() {
	...
	for (int z = -300; z <= 300; z += 55) {

		this->setHexToMesh(this->face, this->frame, glm::vec3(0, 0, z), 300, 3, 50);
	}
}
...
void ofApp::setHexToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height) {
	...
	int deg_start = ofMap(ofNoise(location.z * 0.0005 + ofGetFrameNum() * 0.02), 0, 1, -360, 360);
